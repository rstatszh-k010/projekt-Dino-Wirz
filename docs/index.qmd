...
Die aufbereiteten Daten können nun [hier](https://github.com/rstatszh-k010/projekt-Dino-Wirz/blob/master/daten/processed/daten_komplett.csv) als Textfile betrachtet werden.

# Daten Visualisierung

## Zustand (aktuellste Werte)

Im folgenden sind die Werte aus der letzten vollständig vorliegenden Überwachungsperiode (2015-2019) für den Oberboden abgebildet und in Tabellenform zusammengefasst.
...
filter(kabo,param_txt=="Kupfer",periode == 5) |>
  ...
  scale_x_discrete(name = "Standortnutzung")+
  scale_y_continuous(name = "Gehalt [ppm]",limits = c(0,55),breaks = seq(0,55,10)) +
  labs(title = "Kupfer")

knitr::kable(kabo[1:4,1:4])
...
filter(kabo,param_txt=="Quecksilber",periode == 5) |>
  ...
  theme(panel.background = element_blank(),
        ...
        axis.ticks = element_blank())+
  scale_x_discrete(name = "Standortnutzung")+
  scale_y_continuous(name = "Gehalt [ppm]",limits = c(0,0.25),breaks = seq(0,.25,.05)) +
